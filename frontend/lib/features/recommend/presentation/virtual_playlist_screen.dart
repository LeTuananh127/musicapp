...
class _VirtualPlaylistScreenState extends ConsumerState<VirtualPlaylistScreen> {
  ...
  String get _screenKey =>
      ShuffleStateManager.virtualPlaylistKey(widget.title, widget.artistIds);
  ...
  Future<void> _load() async {
    ...
    try {
      if (widget.tracks != null) {
        _tracks =
            widget.tracks!.map((e) => Map<String, dynamic>.from(e)).toList();
        _prepareChunks();
      } else {
        ...
        final res = await dio.get('$base/artists/tracks',
            queryParameters: {'artists': widget.artistIds!.join(',')});
        final List data =
        ...
        _tracks = data
            .map((e) {
              ...
              return {
                ...
                'cover_url':
                    e['cover_url'] ?? e['cover'] ?? e['album_cover_url'] ?? '',
                'preview_url': preview,
              };
            })
            .toList()
  ...
  Widget build(BuildContext context) {
    final totalMs =
        _tracks.fold<int>(0, (s, e) => s + (e['duration_ms'] as int? ?? 0));
              ...
              : Column(
                  children: [
                    ListTile(
                      title: Text(widget.title),
                      subtitle:
                          Text('${_tracks.length} tracks • Public • $durStr'),
                    ...
                    if (_tracks.isNotEmpty)
                      Padding(
                        ...
                        child: Builder(builder: (c) {
                          ...
                          for (var i = 0;
                              i < _tracks.length && samples.length < 3;
                              i++) {
                            final raw =
                                (_tracks[i]['cover_url'] as String?) ?? '';
                    ...
                    if (_tracks.isEmpty)
                      const Expanded(
                          child: Center(child: Text('Playlist trống')))
                    else
                      Expanded(
                        child: Column(
                          children: [
                            ...
                            Expanded(
                              child: ListView.separated(
                                itemCount: _chunks.isEmpty
                                    ? 0
                                    : _chunks[_pageIndex.clamp(
                                            0, _chunks.length - 1)]
                                        .length,
                                separatorBuilder: (_, __) =>
                                    const Divider(height: 1),
                                itemBuilder: (c, idx) {
                                  final chunk = _chunks.isEmpty
                                      ? <Map<String, dynamic>>[]
                                      : _chunks[_pageIndex.clamp(
                                          0, _chunks.length - 1)];
                                  final t = chunk[idx];
                                  ...
                                  final isCurrent =
                                      player.current?.id == trackModel.id;
                                  final rawCover =
                                      ...
                                      final queue = _shuffled.isNotEmpty
                                          ...
                                          : _tracks
                                              .map((e) => Track(
                                                  ...
                                                  artistName:
                                                      e['artist_name'] ?? '',
                                                  durationMs: (e['duration_ms'] as int?) ?? 0,
