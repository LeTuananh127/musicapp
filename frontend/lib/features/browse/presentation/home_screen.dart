                ...
                itemBuilder: (c, i) {
                  ...
                  return ListTile(
                    ...
                    trailing: PopupMenuButton<String>(
                        ...
                        if (value == 'add') {
                          if (trackId <= 0) {
                            ScaffoldMessenger.of(context).showSnackBar(
                              const SnackBar(
                                  content: Text(
                                      'Track ID không hợp lệ, không thể thêm.')),
                            );
                            return;
                          }
                      ...
                      itemBuilder: (c) => const [
                        PopupMenuItem(
                            value: 'add', child: Text('Thêm vào playlist')),
                      ],
                    ),
                  );
                },
      ...
      builder: (c) {
        return Consumer(builder: (c2, watchRef, _) {
          ...
          final membershipsAsync =
              watchRef.watch(trackPlaylistMembershipsProvider(trackId));
          return SafeArea(
            child: playlistsAsync.when(
              data: (pls) {
                ...
                final memberships = membershipsAsync.maybeWhen(
                    data: (s) => s, orElse: () => <int>{});
                return ListView(
                  children: [
                    ...
                    ListTile(
                      ...
                      onTap: () async {
                        ...
                        try {
                          ...
                          ref.invalidate(
                              trackPlaylistMembershipsProvider(trackId));
                          if (context.mounted) {
                            Navigator.pop(c);
                            ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                                content: Text('Đã tạo và thêm vào "$name"')));
                          }
                        } catch (e) {
                          if (context.mounted) {
                            ScaffoldMessenger.of(context).showSnackBar(
                                SnackBar(content: Text('Lỗi tạo/thêm: $e')));
                          }
                        }
                      },
                    ),
              ...
              ? IconButton(
                  ...
                  onPressed: _loading
                      ...
                      : () async {
                          ...
                          final repo =
                              widget.ref.read(playlistRepositoryProvider);
                          try {
                            await repo
                                .removeTrack(widget.playlistId, widget.trackId)
                            ...
                            try {
                              ...
                              widget.ref.invalidate(
                                  trackPlaylistMembershipsProvider(
                                      widget.trackId));
                            } catch (_) {}
              ...
              try {
                ...
                if (widget.parentContext.mounted) {
                  ...
                  try {
                    ...
                    widget.ref
                        .invalidate(playlistDetailProvider(widget.playlistId));
                    widget.ref.invalidate(
                        trackPlaylistMembershipsProvider(widget.trackId));
                  } catch (_) {}
                ...
                if (widget.parentContext.mounted) {
                  ScaffoldMessenger.of(widget.parentContext).showSnackBar(
                    const SnackBar(
                        content: Text('Hết thời gian chờ máy chủ (timeout).')),
                  );
                }
              } catch (e) {
                if (mounted) setState(() => _loading = false);
