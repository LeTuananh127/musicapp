...
final trackDetailProvider =
    FutureProvider.autoDispose.family<Track?, int>((ref, id) async {
    ...
    return WillPopScope(
      ...
      child: Scaffold(
        ...
        body: async.when(
          ...
          data: (t) {
            if (t == null)
              return const Center(child: Text('Track không tồn tại'));
            ...
            if (audioError != null) {
              WidgetsBinding.instance.addPostFrameCallback((_) {
                ScaffoldMessenger.of(context)
                    .showSnackBar(SnackBar(content: Text(audioError)));
                ...
                children: [
                  ...
                  Builder(builder: (ctx) {
                    ...
                    if (type == 'playlist' && origin['playlistId'] != null) {
                      label = 'Playlist';
                      onOpen = () =>
                          context.go('/playlists/${origin['playlistId']}');
                    } else if (type == 'search') {
                      label = 'Tìm kiếm';
                  ...
                  const SizedBox(height: 8),
                  Text(display.artistName,
                      style: Theme.of(context).textTheme.titleMedium,
                      textAlign: TextAlign.center),
                  const SizedBox(height: 8),
                  // Views
                  if (t.views != null)
                    Text('Lượt xem: ${t.views}',
                        style: Theme.of(context).textTheme.bodyMedium),
                  const SizedBox(height: 16),
                  Text(
                      'Duration: ${display.duration.inMinutes}:${(display.duration.inSeconds.remainder(60)).toString().padLeft(2, '0')}'),
                  ...
                  Row(
                    ...
                    children: [
                      ...
                      IconButton(
                        ...
                        onPressed:
                            state.hasPrevious ? () => ctrl.previous() : null,
                      ),
                        ...
                        onPressed: () {
                          if (display.previewUrl == null) {
                            ScaffoldMessenger.of(context).showSnackBar(
                                const SnackBar(
                                    content: Text(
                                        'Không có preview cho track này.')));
                            return;
                          }
                  ...
                  if (state.current != null) ...[
                    Text(
                        '${_fmt(state.position)} / ${_fmt(state.current!.duration)}'),
                    Slider(
                      value: state.position.inMilliseconds
                          .clamp(0, state.current!.duration.inMilliseconds)
                          .toDouble(),
                      max: state.current!.duration.inMilliseconds == 0
                          ? 1
                          : state.current!.duration.inMilliseconds.toDouble(),
                      onChanged: (v) =>
                  ...
                  const SizedBox(height: 12),
                  if (display.previewUrl != null)
                    Text('Preview URL: ${display.previewUrl}',
